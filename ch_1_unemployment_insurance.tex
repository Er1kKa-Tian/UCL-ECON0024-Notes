...
\section{$\star$ Optimal Unemployment Insurance with No Moral Hazard}
    ...
    \subsection{Expected Utility Model of Unemployment}
        ...
        \begin{itemize}
            ...
            \item $\psi(.)$ is the job searching cost function: we assume it to be \emph{strictly increasing and convex} i.e. $\psi'(.)>0$ and $\psi''(.)>0$ (higher probability of employment $\implies$ higher searching cost)
        \end{itemize}
        
            \subsubsection{Assumptions on the individual side}
                Assume \emphb{no saving/borrowing} and \emphb{homogeneous agents}, we can write the \emphb{individuals’ budget constraints}:
                \begin{itemize}
                ...
                \end{itemize}
                With this assumption, we can express the expected utility as:
                \begin{equation}
            ...
            \subsubsection{Assumptions on the government side:}
                ...
                $$E[U] = (1-p)\times{U \left( w-\frac{p}{1-p}\times{b}\right)} + p\times{U(b)} - \psi(1-p)$$
                Therefore, we can express the \emphb{government's optimisation problem} as:
                \begin{equation}
                    \label{eqn:ui_no_mh}
                    \max_{b} E[U] = (1-p)\times{U\left( w-\frac{p}{1-p}\times{b} \right)} + p\times{U(b)} - \psi(1-p)
                    \end{equation}
        ...
        \subsection{Optimisation Result: Full Insurance}
            ...
            $$\frac{\partial E[U]}{\partial b}=-(1-p)\times{\frac{p}{1-p}}\times{U'\left( w-\frac{p}{1-p}\times{b} \right)} + p\times{U'(b)} = 0$$
            $$U'(b) = U'\left( w-\frac{p}{1-p}\times{b} \right)$$
...
\section{Estimation of Moral Hazard ($\epsilon_{p,b}$)}
    ...
    \subsection{Difference-in-Difference}
        ...
        \end{figure}
        \begin{enumerate}
            \item \emphb{Standard DiD Specification} with 2 states and 2 time period
            ...
            \item \emphb{Fixed Effects Version}\\
        ...
        Meyer (1990) and many others implemented this method using data on unemployment duration in the U.S. and state-level reforms. The general finding shows a \empha{benefit elasticity of 0.4-0.6}: 10\% rise in unemployment benefits leads to about a 4-6\% increase in unemployment duration.
...
\section{Evidence on Consumption Smoothing ($\frac{\Delta c}{c^e}$)}
    ...
    \subsection{Difference-in-Difference}
        ...
        Their results show that: $\beta_0 \approx 0.24$ and $\beta_1 \approx -0.28$, which means \empha{a 10\% increase in UI benefits will induce a 2.8\% reduction in the consumption drop}. Without UI benefits, unemployment will cause a 24\% drop in consumption. With the current level of UI benefit (net benefit replacement rate $\frac{b}{w_i}=0.5$), the consumption is estimated to drop about 10\%.
        
        This provides convincing evidence that insurance markets are not perfect and UI does play a consumption smoothing role.
        
        Note that the drop in consumption to the drop in UI benefit is much less than 1-1. This is because UI also affects people's \emphb{Self Insurance} behaviour:
        \begin{itemize}
...
\section{$\star$ Implication of the Empirical Evidence on the Optimal UI}
    ...
    \subsection{Counter-arguments by Chetty and Szeidl (2007): Consumption Commitment \& Risk Aversion}
        Chetty and Szeidl (2007) pointed out that risk aversion, $\gamma$, is poorly identified, and it appears to vary substantially according to specific situations. Specifically, \empha{people with consumption commitments tend to have higher risk aversions} as explained below.
        ...
        \begin{figure}[H]
            ...
            \label{fig:Chetty}
        \end{figure}
        Shown above, compared with consumption on foods, rents are much harder to be cut off. Therefore, facing unemployment, individuals may need to reduce more flexible consumption, such as foods. In this case, the \empha{actual welfare loss will be amplified}.
...
\section{Should UI Benefits Vary by the Business Cycle?}
    ...
    \subsection{Unemployment Duration and the Business Cycle}
        ...
        \begin{itemize}
            ...
            \item the probability of employment is lower ($(1-p)$ is lower or $p$ is higher)
        \end{itemize}
        Therefore, the \empha{elasticity of unemployment rate with respect to benefits $\epsilon_{p,b} = \frac{b}{p} \frac{dp}{db}$ is lower in recessions}. This in turn indicates lower moral hazard costs, and UI benefits should be higher in recessions to optimise.
        \begin{figure}[H]
            \centering
